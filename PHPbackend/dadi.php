...
if ($userid==-1) { //Narrazione 
	...
	$MySql = "SELECT count(*) FROM dadi WHERE destinatario=-1 OR destinatario=$userid" ;
} else {  // BAH!
...
if ( $count == 0 ) { 		
	...
	if ( $userid ==  -1 ) {
		...
		$MySql = "SELECT * FROM dadi WHERE destinatario=-1 OR destinatario=$userid  ORDER BY ID DESC  ";
	}
